...
function Tweet(props) {
  ...
  async function deleteHandler(twId) {
    await fetch(`http://localhost:3000/tweets/${twId}`, { method: 'DELETE' })
    return history.go(0)
  }
  ...
  return (
    <div className="tweet-container">
      ...
      <div className="tweet-interactions">
        <span ref={commentsCounter} onClick={() => {
          setComents( coments + 1 )
          runAnimation(commentsCounter)
          }}> coments { coments.length }</span>
        <span ref={retweetsCounter} onClick={() => {
          setRetweets( retweets + 1 )
          runAnimation(retweetsCounter)
          }}> retweets</span>
        <span ref={likesCounter} onClick={() => {
          setLikes( likes + 1 )
